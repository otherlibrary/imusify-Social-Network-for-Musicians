...
{
    function asset_js($file, $atts = array())
    {
...
{
    function asset_admin_js($file, $atts = array())
    {
...
{
    function admin_js($file, $atts = array())
    {
...
if(!function_exists('add_meta')){
    function add_meta($meta=array(),$overwrite=false)
    {
        $str = '';
        $ci = &get_instance();
...
if(!function_exists('put_headers')){
    function put_headers()
    {
		...
		$meta=  array(
			array('name' => 'title', 'content' => $ci->config->item('meta_title')),
			array('name' => 'description', 'content' => $ci->config->item('meta_description')),
			array('name' => 'keywords', 'content' => $ci->config->item('meta_keyword')),
			array('name' => 'Content-type', 'content' => $ci->config->item('content_type'), 'type' => 'equiv')      
		);
		$ci->load->helper('html');
		$str.= meta($meta);
