/*
*   JavaScript interface for the imusify Player widget
*   Author: Matas Petrikas, matas@imusify.com
*   Copyright (c) 2009  imusify Ltd.
  ...
  var imusify = window.imusify = {
    ...
    _redispatch: function(eventType, flashId, data) {
      var playerNode,
          ...
          // construct the custom eventType  e.g. 'imusify:onPlayerReady'
          customEventType = 'imusify:' + eventType;
      ...
      // re-dispatch imusify events up in the DOM
      if(window.jQuery){
    ...
    // e.g. imusify.addEventListener('onPlayerReady', myFunctionOne);
    //      imusify.addEventListener('onPlayerReady', myFunctionTwo);
    ...
    // imusify.removeEventListener('onMediaPlay', myFunctionOne);
    ...
    getPlayer: function(id){
      ...
      try{
        if(!id){
          throw "The imusify Widget DOM object needs an id atribute, please refer to imusify Widget API documentation.";
        }
        ...
        if(flash){
          if(flash.api_getFlashId){
            ...
            throw "The imusify Widget External Interface is not accessible. Check that allowscriptaccess is set to 'always' in embed code";
          }
        }else{
          throw "The imusify Widget with an id " + id + " couldn't be found";
        }
      }catch(e){
    ...
    // @flashId: the widget id, basically the Flash node should be accessible to JS with imusify.getPlayer(flashId)
    // @data: an object containing .mediaUri (eg. 'http://api.imusify.com/tracks/49931') .mediaId (e.g. '4532')
